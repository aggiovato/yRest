 ...
 *
 * Accepts three value forms per field:
 * - Plain string  → `{ type: "many2one", target: string }` (level 1 shorthand)
 * - DSL string    → compact `"<type>:<target>[@fk][cardinality][+nested]"` (levels 2–3)
 * - Object        → verbose form with explicit `_type`, `_target`, etc.
 *
...
function parseDslString(raw: string): RelationDef | null {
  ...
  // Split on first colon → type alias + body
  ...
  if (type === "many2many") {
    // Format: target@through(foreignKey,otherKey)
  ...
  // many2one / one2one: target or target@foreignKey
...
/** Strips undefined keys and casts to RelationDef. */
