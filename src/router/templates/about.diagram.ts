...
function collectEdges(){
  ...
  D.relations.forEach(function(r){
    ...
    if(r.type==='many2many'&&r.through&&nIdx[r.through]!=null){
      ...
      if(sn&&tn&&sn!==tn)
        edges.push({a:sn,b:tn,col:col,dashed:false,label:r.label,manyA:r.type!=='one2one',manyB:r.type==='many2many',off:0});
    }
  });
...
function drawLegend(){
  ...
  // Row 2: cardinality symbols (always all shown)
  ...
  x=14;
  var items=[
    {many:true,col:C.m2o,l:'N (many)'},
    {many:false,col:C.m2o,l:'1 (one)'},
    {many:true,col:C.o2o,l:'1:1 both sides'},
    {many:false,col:C.m2m,l:'N:M via pivot'}
  ];
  // Instead show: the four combined pairs
  var pairs=[
  ...
  pairs.forEach(function(p){
    // two badges + short line between them
    var mid=x+32;
...
function draw(){
  ...
  // Pass 1: edge lines + collect badges (parallel edges get perpendicular offsets)
  badges=[];
  ...
  // Pass 3: badges — always rendered on top of nodes
...
document.getElementById('er-dl').addEventListener('click',function(){
  // render at 3× for a sharp PNG regardless of screen DPR
  var SCALE=3;
