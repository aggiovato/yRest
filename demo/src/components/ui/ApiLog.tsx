...
export function ApiLog({ log, emptyMessage = "Try an operation..." }: Props) {
  if (!log) {
    return (
      <div className="api-log">
        <p className="api-log-empty">{emptyMessage}</p>
      </div>
    );
  }
  ...
  const sc =
    log.status >= 200 && log.status < 300 ? "#3fb950" : log.status === 0 ? "#8b949e" : "#f85149";
  const body =
  ...
  return (
    <div className="api-log">
      <div className="api-log-request">
        <span className="log-method" style={{ color: mc }}>
          {log.method}
        </span>
        <span className="log-url">{log.url}</span>
        <span className="log-status" style={{ color: sc }}>
          {log.status || "×"}
        </span>
      </div>
