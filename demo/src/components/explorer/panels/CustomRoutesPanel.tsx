  ...
  return (
    <>
      <p className="feature-desc">
        Static routes defined in the <code>_routes</code> block of <code>db.yml</code>. The response
        is fixed — no server logic. Registered before resource routes, so they take priority over
        any same-name collection.
      </p>
      <div className="btn-try-group">
        <button className="btn-try" onClick={tryHealth}>
          GET /api/health →
        </button>
        <button className="btn-try" onClick={tryLogout}>
