  ...
  return (
    <>
      <p className="feature-desc">
        <code>?_expand=user</code> expands the parent object (todo → user via <code>userId</code>{' '}
      ...
      <div className="btn-try-group">
        <button className="btn-try" onClick={tryExpand}>GET /api/todos?_expand=user →</button>
        <button className="btn-try" onClick={tryEmbed}>GET /api/users/1?_embed=todos →</button>
        <button className="btn-try" onClick={tryNested}>GET /api/users/1/todos →</button>
      </div>
