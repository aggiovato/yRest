  ...
  return (
    <>
      <p className="feature-desc">
        Interpolate request data into the static response body:{' '}
        <code>{'{{params.id}}'}</code>, <code>{'{{now}}'}</code>,{' '}
        <code>{'{{uuid}}'}</code>, <code>{'{{query.x}}'}</code>,{' '}
        <code>{'{{body.x}}'}</code>.
        The <code>uuid</code> is different on every request.
      </p>
      <div className="ex-controls">
        ...
        <button className="btn-try" onClick={handleTry}>GET /api/todos/:id/share →</button>
      </div>
